...
<template>
  <SiteLayout>
    ...
    <div class="welcome-grid ! mx-auto">
      ...
      <section v-if="featuredPosts?.length > 0" class="newest-staff-post">
      ...
      <!-- 3. segundo post "featured" -->
      <section v-if="featuredPosts?.length > 1" class="second-featured">
        <article v-for="(post, index) in featuredPosts?.slice(1, 2)" :key="post.id">
          <Link style="text-decoration: none;" :href="route('post.show', post.slug)">
            <hr>
            <picture>
      ...
      <!-- 5. tercer post "featured" -->
      <section v-if="featuredPosts?.length > 1" class="third-featured">
        <article v-for="(post, index) in featuredPosts?.slice(2, 3)" :key="post.id">
          <Link style="text-decoration: none;" :href="route('post.show', post.slug)">
            <hr>
            <picture>
                <source media="(min-width: 768px)" :srcset="post.thumbnail_urls?.max">
